...
/**
 * News class that contains a topic and a whole bunch of titles of articles
 * based on that topic. Gets its articles from newapi.org.
 * 
 * @author Ryan Razi
 * @version 1.0
 * @since 1.0
...
public class News {
	 ...
	 * 
	 * @return an array of Strings containing news article titles about the topic
	 *         given
	...
	public String[] generateNews() throws IllegalArgumentException {
		...
		// put the valid titles in an ArrayList
		ArrayList<String> validTitles = new ArrayList<>();
		
		// Get information from the website and put it in StringBuilder variable since
		// theres so much text
		...
		try {
			...
			if (responseCode != 200) {
				...
				// Scanner scans the json file it gets from the API and puts in the
				// informationString variable
		...
		// From the articles, we get up to 10 of the titles, and put it into a string
		// array called titles
		for (int i = 0; i < articles.length() && validTitles.size() < 10; i++) {
			JSONObject article = articles.getJSONObject(i);
			String title = article.getString("title");

			// sometimes it gave "[Removed]" for some reason so here we make sure
			// "[Removed]" doesn't get added to the ArrayList
